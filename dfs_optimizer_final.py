...
pitchers = pd.DataFrame({
        ...
        "Projection": [19.5, 18.2, 17.9, 17.6salary_cap = 50000
lineup = []
total_salary = 0
pitcher_added = Falseelse:
        lineup.append(row)
        total_salary += row["Salary"]
    if len(lineup) == 9:
        breaklineup_df = pd.DataFrame(lineup)
st.title("Daily MLB DraftKings Optimizer (Auto Mode)")

# Simulated FanGraphs-style projections for hitters and pitchers
@st.cache_data
def load_sample_projections():})# Combine and sort by projection
...
if row["Position"] == "P":
        if not pitcher_added:
            lineup.append(row)
            total_salary += row["Salary"]
            pitcher_added = Truest.subheader("✅ Optimized DraftKings Lineup")
st.dataframe(lineup_df)hitters = pd.DataFrame({
        ...
        "Projection": [12.4, 12.0, 11.7,hitters, pitchers = load_sample_projections()

st.subheader("Auto-Pulled Hitter Projections")
st.dataframe(hitters)

st.subheader("Auto-Pulled Pitcher Projections")
st.dataframe(pitchers)for _, row in combined_sorted.iterrows():
    if total_salary + row["Salary"] > salary_cap:
        continuecsv = lineup_df.to_csv(index=False).encode("utf-8")
